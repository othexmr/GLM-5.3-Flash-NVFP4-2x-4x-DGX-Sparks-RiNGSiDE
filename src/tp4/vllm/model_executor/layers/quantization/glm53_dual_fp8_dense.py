...
# --- Per-family prefill row chunk (dense-prefill lane 2026-09-24, default OFF) ------------------------------------
# GLM53_DENSE_CHUNK_ROWS="family:rows[:min_rows][,...]" replaces the 2,048-row W8A8 chunk of the named families (rows
# 0 = one call); with min_rows, launches below min_rows rows run as one call (e.g. kda_in:2048:4096 keeps the 2,048-row
# chunks at 4,608 and 13,824 rows but runs a 2,304-row chunk as one call instead of 2,048 + 256). Why: the CUTLASS
# SM120 FP8 kernel launches every tile (no persistent raster), x-fastest along N when there are more M tiles than N
# tiles, so a kda_in weight (26 MB FP8, above L2) is re-streamed per 128-row block; 2,048-row
# chunks avoid that but split every 2,304-row prefill chunk into 2,048 + 256 (the 256-row call takes the 64x64 config,
# 3.4x the per-row cost). Only launches with M > rows loop; the per-token quantization is per row and every chunk
# above 256 rows runs the same 128x128x128 config, so the output is expected to be bit-identical (the leaf checks).
def _chunk_overrides() -> dict[str, tuple[int, int]]:
    raw = os.environ.get("GLM53_DENSE_CHUNK_ROWS", "").strip()
    out: dict[str, tuple[int, int]] = {}
    for token in (t.strip() for t in raw.split(",")):
        if not token:
            continue
        fam, _, rest = token.partition(":")
        rows, _, min_rows = rest.partition(":")
        if fam not in FAMILIES or not rows.strip().isdigit() or (min_rows and not min_rows.strip().isdigit()):
            raise ValueError(f"GLM53_DENSE_CHUNK_ROWS entry {token!r}: need family:rows[:min_rows] with a known family")
        value, floor = int(rows), int(min_rows or 0)
        if value != 0 and (value < 512 or value % 128):
            raise ValueError("GLM53_DENSE_CHUNK_ROWS rows must be 0 or a multiple of 128 that is >= 512")
        out[fam] = (value, floor)
    return out
...
# --- Shared activation quantization (dense-prefill lane 2026-09-24, default OFF) ----------------------------------
# GLM53_DENSE_SHARED_QUANT="producer>consumer[,...]" (e.g. mla_qb>indexer): q_b_proj and the DSA indexer's wq_b take
# the same q_c tensor (mla.py passes one object to both, q_b first), and each quantized it again (267 / 262 us per MLA
# layer at 13,824 rows). For prefill-sized W8A8 launches (M > 128, unchunked), the producer keeps its per-token FP8
# quantization in a one-slot cache keyed by the input tensor object (weak reference) plus its data pointer, shape,
# strides, dtype and device, and the tick of the producing call; the consumer's call reuses it only on an exact match
# and only if no other dense apply() ran in between (its own tick is the producer's + 1), and always clears the slot.
# Same quantization kernel on the same input: bit-identical.
# Fixed 2026-09-24 04:31 (chain 237 crashed in the startup profile run): the key held x._version, which inference
# tensors do not have (vLLM runs the forward under torch.inference_mode()). Without a version counter an in-place write
# to x between producer and consumer cannot be seen at run time, so the pair is safe by construction instead: in the
# served mla.py (80038f1a) q_c goes to q_b_proj (`q = q_proj_layer(q_proj_input)[0]`, q_proj_input is q_c), then only
# `q.view`, the rotary embedding on views of q and k_pe (other tensors) and the indexer call run, and the indexer's
# forward starts with `self.wq_b(qr)` (attention.py c87b31c9 / a681df0e); nothing writes q_c and no other dense
# apply() runs between the two calls. The tick check enforces the second part at run time: a slot left behind (a
# consumer that did not run) or any dense projection in between makes the consumer quantize for itself.
# dense-fp8-kernels lane 2026-09-24: a producer may also be a kernel outside this module that registers the per-token
# quantization of the tensor it hands to the consumer (glm53_register_quant): kda_onorm = the KDA o_norm with the
# o_proj's FP8 quantization fused in (vllm/models/glm5next/nvidia/ops/glm53_onorm_fp8.py, GLM53_KDA_ONORM_FP8).
# prefill-levers lane 2026-09-25 (GLM53_MLA_QNORM_FP8, default off): mla_qnorm = the MLA q_a + kv_a RMSNorm with the
# q_b_proj's FP8 quantization fused in (vllm/models/common/ops/glm53_qnorm_fp8.py, called from mla.py), paired
# as mla_qnorm>mla_qb,mla_qb>indexer: a consumer that is also a producer hands the quantization it took on to its own
# consumer (apply below), so q_b_proj and the indexer's wq_b both read the norm kernel's copy. The served pairs
# (mla_qb>indexer, kda_onorm>attn_out) have no family on both sides, so for them nothing changes.
EXTERNAL_PRODUCERS = ("kda_onorm", "mla_qnorm")


def _shared_quant_pairs() -> dict[str, str]:
    raw = os.environ.get("GLM53_DENSE_SHARED_QUANT", "").strip()
    out: dict[str, str] = {}
    for token in (t.strip() for t in raw.split(",")):
        if not token:
            continue
        producer, _, consumer = token.partition(">")
        if (producer not in FAMILIES and producer not in EXTERNAL_PRODUCERS) or consumer not in FAMILIES \
                or producer == consumer:
            raise ValueError(f"GLM53_DENSE_SHARED_QUANT entry {token!r}: need producer>consumer, two known families")
        out[consumer] = producer
    return out


SHARED_QUANT_CONSUMERS = _shared_quant_pairs()                 # consumer family -> producer family
SHARED_QUANT_PRODUCERS = set(SHARED_QUANT_CONSUMERS.values())
SHARED_QUANT_MIN_M = 128
_SHARED_QUANT_SLOT: dict[str, tuple] = {}                        # producer family -> (key, q, s)
_SHARED_QUANT_TICK = [0]                                          # one tick per dense apply() (and external producer)


def _shared_quant_tick() -> int:
    _SHARED_QUANT_TICK[0] += 1
    return _SHARED_QUANT_TICK[0]


def _shared_quant_key(x: torch.Tensor, tick: int) -> tuple:
    """No version counter: inference tensors have none (see above)."""
    return (weakref.ref(x), x.data_ptr(), tuple(x.shape), tuple(x.stride()), x.dtype, x.device, tick)


def _shared_quant_match(key: tuple, x: torch.Tensor, tick: int) -> bool:
    """The consumer's call has `tick`; the producer's entry must be the tick right before it, on the same object."""
    return key[0]() is x and key[1:] == _shared_quant_key(x, tick - 1)[1:]


_SHARED_QUANT_HITS: dict[str, int] = {}                         # producer family -> reuses so far


def glm53_register_quant(producer: str, x: torch.Tensor, q: torch.Tensor, s: torch.Tensor) -> None:
    """An external producer's per-token FP8 quantization (q [M, K] e4m3, s [M, 1] fp32) of the exact tensor x it hands
    to the consumer next; the consumer reuses it on an exact match and always clears the slot."""
    if producer not in EXTERNAL_PRODUCERS:
        raise ValueError(f"unknown external producer {producer!r}")
    if (x.dim() != 2 or q.shape != x.shape or q.dtype != torch.float8_e4m3fn or s.shape != (x.shape[0], 1)
            or s.dtype != torch.float32):
        raise ValueError("glm53_register_quant: q must be [M, K] e4m3 and s [M, 1] fp32 for a 2-D x [M, K]")
    # The registration takes a tick like a dense apply(): the consumer must be the very next dense apply(). The served
    # caller (kda.py `_glm53_kda_output`) registers and calls `layer.o_proj(x)` in the next statement; RowParallelLinear
    # hands x to apply() unchanged (input_is_parallel), so nothing can write x in between (see the note above).
    _SHARED_QUANT_SLOT[producer] = (_shared_quant_key(x, _shared_quant_tick()), q, s)


def glm53_consumes_shared_quant(layer: torch.nn.Module, m: int, producer: str) -> bool:
    """True iff apply(layer, x) for an x of m rows takes the unchunked W8A8 branch that reuses `producer`'s slot (the
    external producer's precondition: quantize only where the consumer is certain to take the quantization)."""
    method = getattr(layer, "quant_method", None)
    if not isinstance(method, DualPathFp8DenseLinearMethod) or method.prefill_mode != "w8a8":
        return False
    if SHARED_QUANT_CONSUMERS.get(method.family) != producer or m <= max(SHARED_QUANT_MIN_M, MARLIN_MAX_M):
        return False
    hn = getattr(layer, "dual_humming_nvfp4", None)
    if hn is not None and m in hn["kernels"]:
        return False
    chunk = layer.dual_chunk_rows
    return not (chunk and m > chunk and m >= getattr(layer, "dual_chunk_min_rows", 0))


# --- SM120 FP8 GEMM configs (dense-fp8-kernels lane 2026-09-24, default OFF) -----------------------------------------
# GLM53_DENSE_FP8_GEMM="family=config/swizzle/raster@min_rows[-max_rows],..." sends the prefill W8A8 GEMMs of a family
# whose call has min_rows..max_rows rows (min_rows > 128: never at decode) to a config of the glm53_fp8_gemm extension
# (GLM53_FP8_GEMM_SO = the built .so; outputs/2026-09-24-dense-fp8-kernels/cutlass): vLLM's SM120 FP8 kernel with its
# epilogue tree verbatim and another CTA tile / schedule / stage count, and the tile scheduler's swizzle (1, 2, 4, 8) and
# raster (0 heuristic, 1 along M, 2 along N). Same MMA atom, same k order, same epilogue: bit-identical to
# _C.cutlass_scaled_mm (the leaf checks it). First matching entry wins; bias-free calls only; anything else runs the
# served kernel.
def _fp8_gemm_rules() -> list[tuple]:
    raw = os.environ.get("GLM53_DENSE_FP8_GEMM", "").strip()
    rules = []
    for token in (t.strip() for t in raw.split(",")):
        if not token:
            continue
        try:
            fam, spec_rows = token.split("=", 1)
            spec, rows = spec_rows.split("@", 1)
            name, swizzle, raster = spec.split("/")
            lo, _, hi = rows.partition("-")
            rule = (fam, int(lo), int(hi) if hi else 1 << 31, name, int(swizzle), int(raster))
        except ValueError:
            raise ValueError(f"GLM53_DENSE_FP8_GEMM entry {token!r}: need family=config/swizzle/raster@min[-max]") from None
        if fam not in FAMILIES or rule[1] <= 128 or rule[2] < rule[1] or rule[4] not in (1, 2, 4, 8) \
                or rule[5] not in (0, 1, 2) or not name:
            raise ValueError(f"GLM53_DENSE_FP8_GEMM entry {token!r}: unknown family, min_rows <= 128 or bad swizzle/raster")
        rules.append(rule)
    return rules


FP8_GEMM_RULES = _fp8_gemm_rules()
_FP8_GEMM: list = []                                            # [module, {config name: index}] once loaded


def _fp8_gemm_choice(family: str, m: int, n: int, k: int, bias) -> tuple | None:
    if bias is not None or n % 16 or k % 16:
        return None
    for fam, lo, hi, name, swizzle, raster in FP8_GEMM_RULES:
        if fam == family and lo <= m <= hi:
            if not _FP8_GEMM:
                import importlib.machinery
                import importlib.util

                path = os.environ["GLM53_FP8_GEMM_SO"]
                loader = importlib.machinery.ExtensionFileLoader("glm53_fp8_gemm", path)
                spec = importlib.util.spec_from_file_location("glm53_fp8_gemm", path, loader=loader)
                mod = importlib.util.module_from_spec(spec)
                loader.exec_module(mod)
                names = {n_: i for i, n_ in enumerate(mod.configs())}
                missing = {r[3] for r in FP8_GEMM_RULES} - set(names)
                if missing:
                    raise ValueError(f"GLM53_DENSE_FP8_GEMM: {path} has no config {sorted(missing)}")
                _FP8_GEMM[:] = [mod, names]
            logger.info_once("GLM53 dense FP8 GEMM config %s (swizzle %d, raster %d) for family %s", name, swizzle,
                             raster, family, scope="local")
            return _FP8_GEMM[1][name], swizzle, raster
    return None
...
class DualPathFp8DenseLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.dual_chunk_min_rows = 0
        if self.family in CHUNK_OVERRIDES:
            layer.dual_chunk_rows, layer.dual_chunk_min_rows = CHUNK_OVERRIDES[self.family]
            logger.info_once("GLM53 dense chunk override: family %s prefill row chunk %d from %d rows (0 = one call)",
                             self.family, layer.dual_chunk_rows, layer.dual_chunk_min_rows, scope="local")
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if chunk and M > chunk and M >= getattr(layer, "dual_chunk_min_rows", 0):
            ...
            for i in range(0, M, chunk):
                ...
                cfg = _fp8_gemm_choice(self.family, xs.shape[0], N, K, bias) if FP8_GEMM_RULES else None
                if cfg is not None:
                    _FP8_GEMM[0].gemm(out[i : i + xs.shape[0]], q, w8t, s, layer.dual_w8_scale, *cfg)
                else:
                    torch.ops._C.cutlass_scaled_mm(out[i : i + xs.shape[0]], q, w8t, s, layer.dual_w8_scale, bias)
        else:
            q = s = None
            if M > SHARED_QUANT_MIN_M and self.family in SHARED_QUANT_CONSUMERS:
                slot = _SHARED_QUANT_SLOT.pop(SHARED_QUANT_CONSUMERS[self.family], None)
                if slot is not None and _shared_quant_match(slot[0], x, tick):
                    q, s = slot[1], slot[2]
                    producer = SHARED_QUANT_CONSUMERS[self.family]
                    _SHARED_QUANT_HITS[producer] = _SHARED_QUANT_HITS.get(producer, 0) + 1
                    logger.info_once("GLM53 dense shared quant: family %s reuses the %s activation quantization",
                                     self.family, SHARED_QUANT_CONSUMERS[self.family], scope="local")
            if q is None:
                q, s = ops.scaled_fp8_quant(x2, use_per_token_if_dynamic=True)
                if M > SHARED_QUANT_MIN_M and self.family in SHARED_QUANT_PRODUCERS:
                    _SHARED_QUANT_SLOT[self.family] = (_shared_quant_key(x, tick), q, s)
            elif self.family in SHARED_QUANT_PRODUCERS:
                # prefill-levers 2026-09-25: the relay (a consumer that is also a producer, mla_qnorm>mla_qb>indexer):
                # the taken quantization of the same x, keyed on this call's tick like the producer branch above.
                _SHARED_QUANT_SLOT[self.family] = (_shared_quant_key(x, tick), q, s)
            cfg = _fp8_gemm_choice(self.family, M, N, K, bias) if FP8_GEMM_RULES else None
            if cfg is not None:
                out = torch.empty((M, N), dtype=x2.dtype, device=x2.device)
                _FP8_GEMM[0].gemm(out, q, w8t, s, layer.dual_w8_scale, *cfg)
            else:
