...
class SchedulerBridge:
    def __init__(self, scheduler):
        ...
        if self.adaptive_chunk and (not self.split_fair or not self.fair_prefill
                or self.runtime_prefill or self.prefill_cap != 4608):
            raise ValueError("adaptive chunk requires static split/fair4608")
        if self.adaptive_chunk:
            self.fair_prefill.validate_cap(2304)
            log.info("GLM53_ADAPTIVE_FAIR_CHUNK selected=True decoder_threshold=5 low=2304 high=4608")
    ...
    def _validate_prefill_adaptive(self, setting, cap):
        if self.split_fair:
            if (setting.mode not in ("off", "ema", "forced", "ema4", "saturation") or (setting.mode != "forced" and setting.forced_k != self.max_k)
                    or self.scope != "all" or cap != 4608 or not self.fair_prefill
                    or self.split_adaptive or not self.prefill_adaptive
                    or self.runtime_prefill or self.cost_reload or self.costs_path):
                raise ValueError("split/fair requires all capture, static fair4608 and off/EMA/forced/ema4/saturation only")
        ...
        if (not self.prefill_adaptive or setting.mode not in ("ema", "forced", "ema4", "saturation")
                or self.scope != "all" or cap != 4608 or self.runtime_prefill
                or self.cost_reload or self.costs_path
                or (setting.mode in ("ema", "ema4", "saturation") and setting.forced_k != self.max_k)):
            raise ValueError("prefill/adaptive composition requires explicit opt-in, static cap4608, all capture and EMA/forced/ema4/saturation only")
...
def order_running(scheduler):
    ...
    if getattr(state, "fair_prefill", None):
        ...
        effective_cap = chunk_cap(state.prefill_cap, decoder_count, state.adaptive_chunk, max_requests=state.max_requests)
