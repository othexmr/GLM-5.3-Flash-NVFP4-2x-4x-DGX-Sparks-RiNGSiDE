...
from vllm.entrypoints.openai.responses.content_logprobs import content_logprob_span
...
class OpenAIServingResponses(GenerateBaseServing):
    ...
    async def responses_full_generator(
        ...
        *,
        ...
        if self.use_harmony:
            ...
            if context.response_parser and getattr(context.response_parser, "tool_call_truncated", False):
                status = "incomplete"
        ...
        if (
            ...
            and context.response_parser
            and getattr(context.response_parser, "tool_call_truncated", False)
            and context.final_output.outputs[0].finish_reason != "length"
        ):
            # A model-stopped malformed call is not a token-budget exhaustion.
        ...
        *,
        ...
        if parser:
            ...
            if not request.include_reasoning:
                reasoning = None
                logprobs = None
            logprobs, _ = content_logprob_span(
                final_output.text, content, logprobs or []
            )
            ...
            if getattr(parser, "tool_call_truncated", False):
    ...
    async def _process_simple_streaming_events(
        ...
        async for ctx in result_generator:
            ...
            raw_text += delta_text
            raw_logprobs.extend(_get_logprobs(output))
            ...
            for dm in split_delta(delta_message):
                ...
                aligned_logprobs = []
                if dm.content:
                    aligned_logprobs, content_cursor = content_logprob_span(
                        raw_text, dm.content, raw_logprobs, content_cursor
                    )
                for event in processor.emit_delta(
                    dm, output, lambda _: aligned_logprobs
                ):
                    yield _increment_sequence_number_and_return(event)
    ...
    async def responses_stream_generator(
        ...
        async with AsyncExitStack() as exit_stack:
            ...
            yield _increment_sequence_number_and_return(
                terminal_event(
                    type="response.incomplete" if final_response.status == "incomplete" else "response.completed",
                    sequence_number=-1,
