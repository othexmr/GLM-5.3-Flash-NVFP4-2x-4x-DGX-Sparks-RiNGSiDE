...
class SimpleStreamingState:
    ...
    content_logprobs_complete: bool = True
    tool_call_id: str = ""
    tool_call_name: str = ""
...
def emit_simple_content_open(
    ...
    state.content_logprobs_complete = True
    return [
        ResponseOutputItemAddedEvent(
            type="response.output_item.added",
            sequence_number=-1,
...
def emit_simple_content_delta(
    ...
    if delta and "".join(entry.token for entry in logprobs or []) != delta:
        state.content_logprobs_complete = False
        state.accumulated_logprobs = []
    if state.content_logprobs_complete:
...
class SimpleStreamingEventProcessor:
    ...
    def close_current(self, *, tool_call_truncated: bool = False) -> list[StreamingResponsesResponse]:
        """Close the current state and emit its 'done' event sequence."""
        handlers = self._STATE_HANDLERS.get(self.state.current_state)
        if handlers is None:
            return []
        closing_tool = self.state.current_state == _StateType.TOOL_CALL
        events = handlers.done_fn(self.state)
        if tool_call_truncated and closing_tool:
            for event in events:
