...
class SingleTypeKVCacheManager(ABC):
    ...
    def cache_blocks(
        ...
        # SCHED-KDA-01: drop blocks whose stored contents do not correspond to
        # the token count they would be registered under.
        block_mask = self._mask_unwritten_state_blocks(
            request, num_cached_blocks, num_full_blocks, block_mask
        )
    ...
    def _mask_unwritten_state_blocks(
        self,
        request: Request,
        start_block: int,
        end_block: int,
        ...
        """Hook: drop blocks whose contents do not match their hash label.

        Attention groups write every token of a block before it is declared
        full, so the base policy is the identity. Recurrent (Mamba/KDA) groups
        store one state per slot, written at a chunk end, and override this.
        """
        return block_mask
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def __init__(
        ...
        # SCHED-KDA-01: per request, the state-slot indices left holding a
        # *non-boundary* state by a chunked-prefill chunk that ended mid-block.
        # Slot p is only a legal prefix-cache entry while it holds the state
        # after exactly (p + 1) * block_size tokens.
        self._partial_state_block_idx: dict[str, set[int]] = {}
    ...
    def _record_state_write_position(self, request: Request, num_tokens: int) -> None:
        """Remember whether this step leaves the running slot off a boundary.

        The worker writes the running state into slot
        ``cdiv(num_tokens, block_size) - 1`` and only *materialises a block
        boundary* when the step's aligned end has already reached the running
        state position -- during chunked prefill that means the chunk end is a
        multiple of ``block_size`` (``needs_copy``, mamba_utils.py:446-470).
        A prefill chunk that ends mid-block therefore leaves that slot short of
        the boundary it would later be hashed under, with nothing to repair it.
        The prompt's last chunk and every decode step are exempt: the slot
        advances to its boundary as decode proceeds, which the worker does
        materialise.
        """
        if num_tokens <= 0:
            return
        idx = num_tokens // self.block_size
        partial_idx = self._partial_state_block_idx.setdefault(
            request.request_id, set()
        )
        if num_tokens % self.block_size == 0:
            partial_idx.discard(idx - 1)
        elif num_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
            partial_idx.add(idx)
    ...
    def _mask_unwritten_state_blocks(
        ...
        """Refuse to register a state slot under a boundary it does not hold.

        A recurrent slot stores a single state, written at a chunk end. If a
        prefill chunk ended mid-block the slot is short of
        ``(p + 1) * block_size``, and a later chunk that runs past that
        boundary must not hash it there: a hit would resume decoding from a
        state that trails its own full-attention context. Masking turns a
        silently wrong hit into a missed hit.
        """
        partial_idx = self._partial_state_block_idx.get(request.request_id)
        if not partial_idx:
            return block_mask
        stale = [i for i in partial_idx if start_block <= i < end_block]
        if not stale:
            return block_mask
        if block_mask is None:
            block_mask = [True] * (end_block - start_block)
        for i in stale:
            block_mask[i - start_block] = False
        return block_mask
    ...
    def pop_blocks_for_free(self, request_id: str) -> list[KVCacheBlock]:
        if self.mamba_cache_mode == "align":
            ...
            self._partial_state_block_idx.pop(request_id, None)
