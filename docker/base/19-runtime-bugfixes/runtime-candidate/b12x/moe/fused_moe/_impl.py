...
# memory route resolution for routed_rows <= B12X_GLM53_DECODE_FASTPATH_MAX_ROWS
# (default 64).  Off unless B12X_GLM53_DECODE_FASTPATH=1; both read once at import.
_GLM53_DECODE_FASTPATH_ENV = "B12X_GLM53_DECODE_FASTPATH"
_GLM53_DECODE_FASTPATH_MAX_ROWS_ENV = "B12X_GLM53_DECODE_FASTPATH_MAX_ROWS"
...
def _resolved_dynamic_swap_ab(quant_mode: str, activation: str, n: int) -> bool:
    """Use the same resolved layout for kernel selection and construction."""
    quant_mode = _normalize_quant_mode(quant_mode)
    if _is_w4a8_quant_mode(quant_mode) or quant_mode in _W6A8_QUANT_MODES:
        return False
    override = os.environ.get("B12X_DYNAMIC_SWAP_AB")
    if override is not None:
        return override != "0"
    return bool(
        quant_mode == "nvfp4"
        and _get_activation_kernel_spec(activation, quant_mode=quant_mode).is_gated
        and int(n) % 128 != 0
        and int(n) % 32 == 0
    )
...
def _glm53_decode_fastpath_selected(
    *,
    ...
    grouped (not direct) routing on the materialized queue,
    ...
    return bool(
        ...
        and int(n) % 128 == 0
        and not _resolved_dynamic_swap_ab(quant_mode, activation, n)
...
def _get_dynamic_kernel(
    ...
    *,
    ...
    swap_ab = _resolved_dynamic_swap_ab(quant_mode, activation, n)
