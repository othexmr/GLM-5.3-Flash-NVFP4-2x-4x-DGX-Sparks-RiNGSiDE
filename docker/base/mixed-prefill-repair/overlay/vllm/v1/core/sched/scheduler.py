...
from glm53_speedup.scheduler import bridge as _speedup_bridge, prefill_limit as _speedup_prefill_limit, order_running as _speedup_order_running, alignment_limit as _speedup_alignment_limit
import itertools
import time
...
class Scheduler(SchedulerInterface):
    ...
    def _mamba_block_aligned_split(
        ...
        # SCHED-KDA-01 is a reusable-checkpoint invariant. With retention
        # disabled, keep the qualified prefill partition: changing KDA chunk
        # boundaries changes floating-point state even on a cold request.
        retain_checkpoints = (
            getattr(self.cache_config, "enable_prefix_caching", True)
            and getattr(self.cache_config, "prefix_cache_retention_interval", None) != 0
        )
        block_size = (
            self.block_size if retain_checkpoints else self.cache_config.block_size
        )
        last_cache_position = request.num_tokens - request.num_tokens % block_size
        if self.use_eagle:
            if retain_checkpoints:
                # Drop one drafter block, then floor to a KDA boundary.
                eagle_unit = self.cache_config.block_size
                last_cache_position = max(
                    (request.num_tokens - eagle_unit) // block_size * block_size, 0
                )
            else:
        ...
        if end < prefill_end and not use_internal_checkpoint:
            max_prefill_tokens = _speedup_alignment_limit(self, request, self.max_num_scheduled_tokens)
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        _speedup_bridge(self).select(self, num_scheduled_tokens, scheduled_spec_decode_tokens, preempted_reqs)
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if scheduled_spec_token_ids and (
                ...
                _speedup_bridge(self).observe(req_id, num_draft_tokens, num_accepted, output_is_stale)
