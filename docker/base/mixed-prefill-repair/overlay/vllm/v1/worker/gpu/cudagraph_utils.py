...
from glm53_speedup.graphs import extend_candidates as _speedup_graphs, capture_start as _speedup_capture_start, capture_end as _speedup_capture_end, eligible_descriptor as _speedup_eligible
import torch
...
class CudaGraphManager:
    ...
    def _init_candidates(self) -> None:
        ...
        _speedup_graphs(self, descs_by_mode, BatchExecutionDescriptor, CUDAGraphMode)
    ...
    def dispatch(
        ...
        if self._graphs_captured and num_tokens > 0 and key in self._candidates:
            for desc in self._candidates[key]:
                if not _speedup_eligible(desc, num_reqs, num_tokens, uniform_token_count, max_query_len):
                    continue
                if _is_compatible(
                    desc,
                    num_reqs,
