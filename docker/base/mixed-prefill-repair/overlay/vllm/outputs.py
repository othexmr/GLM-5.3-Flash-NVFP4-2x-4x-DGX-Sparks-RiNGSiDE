...
class RequestOutput:
    ...
    def add(self, next_output: "RequestOutput", aggregate: bool) -> None:
        ...
        # These are cumulative snapshots, including final timing/cache values.
        # Absent metadata must not erase an earlier populated snapshot.
        if next_output.metrics is not None:
            self.metrics = next_output.metrics
        if next_output.num_cached_tokens is not None:
            self.num_cached_tokens = next_output.num_cached_tokens
        if next_output.num_cache_creation_tokens is not None:
            ...
            for i, completion in enumerate(self.outputs):
                if completion.index == next_completion.index:
                    if aggregate:
                        ...
                        # Terminal per-sequence totals are snapshots, not deltas.
                        # Do not sum them or erase them on a metric-free update.
