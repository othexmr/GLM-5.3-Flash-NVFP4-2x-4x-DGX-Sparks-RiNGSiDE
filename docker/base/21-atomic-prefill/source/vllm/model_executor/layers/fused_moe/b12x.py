...
_GLM53_ATOMIC_PREFILL = os.environ.get("B12X_GLM53_ATOMIC_PREFILL", "0") == "1"
_GLM53_PREFILL_POLICY_LOGGED: set[str] = set()


def _glm53_use_atomic_prefill(tokens: int, quant_mode: str) -> bool:
    """Only eager, pure-prefill NVFP4 batches; mixed/unknown/graphs keep baseline."""
    if not _GLM53_ATOMIC_PREFILL or quant_mode != "nvfp4" or tokens < 128:
        return False
    if not is_forward_context_available() or _is_current_stream_capturing():
        return False
    ctx = get_forward_context()
    # Log each metadata/mode combination once, outside CUDA capture.
    metadata = ctx.attn_metadata
    values = list(metadata.values()) if isinstance(metadata, dict) else []
    signature = "metadata:" + repr((str(ctx.cudagraph_runtime_mode), sorted({
        (type(m).__name__, str(getattr(m, "glm53_moe_all_prefill", "unmarked")))
        for m in values
    })))
    if signature not in _GLM53_PREFILL_POLICY_LOGGED:
        logger.info("GLM53_MOE_PHASE_METADATA tokens=%d %s", tokens, signature)
        _GLM53_PREFILL_POLICY_LOGGED.add(signature)
    # FULL replay bypasses this function. Never capture an atomic plan into it.
    from vllm.config import CUDAGraphMode
    if ctx.cudagraph_runtime_mode != CUDAGraphMode.NONE:
        return False
    if not isinstance(metadata, dict) or not metadata:
        return False
    # The dictionary also contains the dense drafter's attention metadata.
    # Scheduler phase is global to the target batch; its NoPE builders must
    # agree. The drafter need not expose this target-only policy marker.
...
class B12xExperts(mk.FusedMoEExpertsModular):
    ...
    def __init__(
        ...
        self._plans: dict[tuple[int, int, MoEActivation, bool, bool], Any] = {}
    ...
    def _plan(
        ...
        *,
        ...
        atomic_prefill = _glm53_use_atomic_prefill(int(tokens), self._quant_mode)
        key = (
            max(int(tokens), 1),
            int(topk),
            activation,
        ...
        plan = fused_moe.plan(
            fused_moe.Caps(
                ...
                deterministic_output=False if atomic_prefill else None,
                route_num_experts=0,
                quant_mode=self._quant_mode,
        ...
        if _GLM53_ATOMIC_PREFILL:
            policy = "atomic-prefill" if atomic_prefill else "baseline"
            if policy not in _GLM53_PREFILL_POLICY_LOGGED:
