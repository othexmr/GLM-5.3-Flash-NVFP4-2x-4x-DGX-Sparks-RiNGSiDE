...
for data in ({'nested':[1,{'text':'quote " slash \\ line\n ไทย'}],'null':None}, {'unicode':'日本語 😀','bool':True}):
 expected=[{'name':'inspect','parameters':{'data':data}}]
 ...
 for chunks in layouts:
  p=cls(tok,tools=request.tools,chat_template_kwargs={});calls={};reason=[]
  try:
   ...
   assert ''.join(reason)=='Inspecting.';assert len(calls)==1,calls
