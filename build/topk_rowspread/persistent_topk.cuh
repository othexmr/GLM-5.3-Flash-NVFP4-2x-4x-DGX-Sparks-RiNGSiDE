// Modified by the GLM-5.3 RiNGSiDE recipe (othexmr): rowspread: every row at or below RADIX_THRESHOLD is taken by
// one CTA of the whole grid in a first pass; the radix rows follow as before (build/topk_rowspread/README.md).
/*
...
__global__ void __launch_bounds__(kThreadsPerBlock, 2)
  ...
  // [rowspread, ctx1m-topk 2026-09-25] Pass 1: every row at or below
  // RADIX_THRESHOLD is taken by one CTA of the whole grid (row r -> CTA
  // r mod total), instead of by CTA 0 of its group only; the other CTAs of a
  // group used to skip such rows, so at the served strides only 1-4 CTAs
  // worked on short rows. The per-row code and the row-length clamp are the
  // original ones. Pass 2 below is the original loop restricted to the radix
  // rows: the same groups, the same static round-robin, the same radix_iter
  // sequence. Every CTA finishes pass 1 before its first radix barrier; the
  // host keeps the cooperative launch co-resident, so the barriers still
  // complete.
  {
    const uint32_t total_ctas = num_groups * ctas_per_group;
    const uint32_t row_bound_1 =
    ...
    for (uint32_t row_idx = blockIdx.x; row_idx < params.num_rows;
      ...
      const uint32_t seq_len =
          non_negative_len < row_bound_1 ? non_negative_len : row_bound_1;
      if (seq_len > RADIX_THRESHOLD) continue;
      ...
      // No thread may start the next row's shared-memory initialisation
      // while another still reads this row's scalars.
      __syncthreads();
    }
  }

  // Early exit: no row can reach the radix path (every length is clamped to
  // max_seq_len), so pass 2 has nothing to do.
  if (params.max_seq_len <= RADIX_THRESHOLD) return;
  ...
  for (uint32_t iter = 0; iter < total_iters; iter++) {
    ...
    // [rowspread] short rows were done in pass 1.
    if (seq_len <= RADIX_THRESHOLD) continue;
